...
export default function Reservation() {
    ...
    const { startdayText, enddayText, DeleteYear, resetFilters, setBackCheck } = useContext(CalendarContext);
    ...
    useEffect(() => {
        axios.get(`/api/userinfo/${userID}`, { userId: userID })
            .then((res) => {
                ...
                if (filters?.endDate === null) { // endDate == null => 예약완료 된 후 뒤로가기로 결제 페이지 접근
                    navigate(+1); // URL 앞으로가기
                    setBackCheck(true); // 뒤로가기 체크키 true로 지정
                }
            })
            .catch((error) => {
                console.log(error)
